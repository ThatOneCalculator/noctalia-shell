    ...
    Item {
      ...
      Row {
        ...
        NText {
          ...
          // If hovered or just switched window, show up to 350 pixels
          // If not hovered show up to 180 pixels
          width: (showingFullTitle || mouseArea.containsMouse) ? Math.min(fullTitleMetrics.contentWidth,
                                                                          350 * scaling) : Math.min(
                                                                   fullTitleMetrics.contentWidth, 180 * scaling)
          text: getTitle()
